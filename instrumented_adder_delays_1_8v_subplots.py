...
def plot():
    ...
    ax_rca_vs_csa = axes[0]
    ax_all_vs_csa = axes[1]

    legend_handles = {}
    # Filter for 'behavioural' and 'ripple' adders
    filtered_adders_1 = {k: adders[k] for k in ['behavioural', 'ripple']}
    for adder_name, adder_info in filtered_adders_1.items():
        lines = ax_rca_vs_csa.plot(bit_distance, adder_info["data"]["delay_1.8v"],
        ...
        # Take the first element from lines to get the Line2D object
        line = lines[0]
        if adder_info["plot_style"]["label"] not in legend_handles:
            legend_handles[adder_info["plot_style"]["label"]] = line

    # Filter for 'behavioural', 'kogge', 'sklansky', and 'brent_kung' adders
    filtered_adders_2 = {k: adders[k] for k in ['behavioural', 'kogge', 'sklansky', 'brent_kung']}
    for adder_name, adder_info in filtered_adders_2.items():
        lines = ax_all_vs_csa.plot(bit_distance, adder_info["data"]["delay_1.8v"],
                                   marker=adder_info["plot_style"]["marker"],
                                   linestyle=adder_info["plot_style"]["linestyle"],
                                   color=adder_info["plot_style"]["color"],
                                   label=adder_info["plot_style"]["label"])
...
#    filtered_adders_1 = {k: adders[k] for k in ['behavioural', 'ripple']}
#    for adder_name, adder_info in filtered_adders_1.items():
#        tmp = ax_rca_vs_csa.plot(bit_distance, adder_info["data"]["delay_1.8v"],
#                           marker=adder_info["plot_style"]["marker"],
#                           linestyle=adder_info["plot_style"]["linestyle"],
#                           color=adder_info["plot_style"]["color"],
#                           label=adder_info["plot_style"]["label"])
#        if adder_info["plot_style"]["label"] not in legend_handles:
#            legend_handles[adder_info["plot_style"]["label"]]=tmp
#
#    # Filter for 'behavioural', 'kogge', 'sklansky', and 'brent_kung' adders
#    filtered_adders_2 = {k: adders[k] for k in ['behavioural', 'kogge', 'sklansky', 'brent_kung']}
#    for adder_name, adder_info in filtered_adders_2.items():
#        ax_all_vs_csa.plot(bit_distance, adder_info["data"]["delay_1.8v"],
#                           marker=adder_info["plot_style"]["marker"],
#                           linestyle=adder_info["plot_style"]["linestyle"],
#                           color=adder_info["plot_style"]["color"],
#                           label=adder_info["plot_style"]["label"])
...
#            legend_handles[adder_info["plot_style"]["label"]]=tmp
    ...
    # customize ticks
    ax_rca_vs_csa.set_xticks(bit_distance)
    ax_rca_vs_csa.set_xticklabels([f"{bd}" for bd in bit_distance])
    ax_all_vs_csa.set_xticklabels([])

    # legend and labels
    fig.supxlabel("Index of Output Bit")
    fig.supylabel(r"Delay $(ns)$")
    #ax.set_title("Comparison of Adder Topologies at 1.8V")

    handles, labels = zip(*legend_handles.items())
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=len(adders), title="Adder Topology", fancybox=False, framealpha=1.0, edgecolor="white")
    ...
    #plt.tight_layout()
    plt.savefig(f"instrumented_adder_delays_1_8v_subplots.pdf", bbox_inches='tight')
